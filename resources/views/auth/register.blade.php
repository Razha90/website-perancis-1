<x-guest-layout>
    <form method="POST" action="{{ route('register') }}">
        ...
        <div class="flex items-end justify-between mt-4">
            <a class="underline text-sm text-secondary_blue opacity-85" href="{{ route('login') }}">
                {{ __('register.already_have_an_account') }}
            </a>
            <button type="submit"
                class="rounded-md px-4 py-2 bg-accent_blue text-primary_white text-base hover:opacity-25 transition-opacity ">{{ __('register.submit') }}</button>
        </div>
    </form>
</x-guest-layout>